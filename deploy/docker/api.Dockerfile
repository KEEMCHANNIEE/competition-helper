# =============================================================================
# api.Dockerfile — keenee의 FastAPI 서버(현관) 이미지
# =============================================================================
# 이 파일이 무엇인가:
...
#   api 는 uv workspace 멤버라서 공유 라이브러리 libs/keenee_core 에 의존한다.
#   따라서 Docker 빌드 컨텍스트는 반드시 "레포 루트(keenee/)" 여야 한다.
#   (서비스 폴더 안이 아니라 루트에서 빌드해야 libs/ 를 COPY 할 수 있음)
#
#   올바른 빌드 명령 (레포 루트에서 실행):
#       docker build -f deploy/docker/api.Dockerfile -t keenee-api .
#                                                                   ^ 마지막 점(.) = 컨텍스트 = 레포 루트
...
ENV PYTHONUNBUFFERED=1 \
    ...
    PYTHONPATH=/app/services/api/src:/app/libs/keenee_core/src
...
#   2) libs/keenee_core/    → api 가 의존하는 공유 계약 계층
...
#   --package  : 워크스페이스 중 api 패키지(+그 의존 keenee-core)만 설치
# ※ 한 번이라도 `uv lock` 을 돌려 uv.lock 을 커밋한 뒤에는,
#   재현 가능한 빌드를 위해 아래 줄을 `uv sync --no-dev --frozen --package keenee-api` 로 바꿀 것.
RUN uv sync --no-dev --package keenee-api

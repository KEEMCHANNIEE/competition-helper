...
- Redis 리스트 키: ``keenee:jobs:recommend``
- payload: ``RecommendJobPayload`` JSON
...
from keenee_core.db import get_engine
...
QUEUE_KEY = "keenee:jobs:recommend"
...
def _get_or_create_job(session: Session, payload: RecommendJobPayload) -> AgentJob:
...
def run_loop(
    *,
    ...
    while max_iterations is None or iterations < max_iterations:
        iterations += 1
        item = redis.brpop(queue_key, timeout=BRPOP_TIMEOUT)
        if not item:
            continue  # 타임아웃 — 다음 바퀴.
        _key, raw = item
        payload = RecommendJobPayload.model_validate_json(raw)
        handle_job(
            payload,

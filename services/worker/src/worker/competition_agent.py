...
def _format_detail(d) -> str:
    """공모전 DB가 가진 모든 정보를 빠짐없이 사람이 읽기 좋게 정리한다.

    "자세히 알려줘" 요청의 응답 본문이므로, 핵심 항목은 값이 없어도 "정보 없음"으로
    표기하고, 부가 항목(주최·키워드·설명·링크 등)은 값이 있을 때만 줄을 만든다.
    """
    lines = [d.title]

    org = d.organizer or d.host
    if org:
        host_type = f" ({d.host_type})" if getattr(d, "host_type", None) else ""
        lines.append(f"주최·주관: {org}{host_type}")
    if d.category:
        lines.append(f"분야: {', '.join(d.category)}")
    if getattr(d, "keywords", None):
        lines.append(f"키워드: {', '.join(d.keywords)}")
    if getattr(d, "target", None):
        lines.append(f"참가 대상: {', '.join(d.target)}")

    deadline = d.deadline.isoformat() if d.deadline else "정보 없음"
    if getattr(d, "start_date", None):
        lines.append(f"접수 기간: {d.start_date.isoformat()} ~ {deadline}")
    else:
    ...
    prize_parts = []
    if d.total_prize_amount:
        prize_parts.append(f"총상금 {d.total_prize_amount:,}원")
    if getattr(d, "first_prize_amount", None):
        prize_parts.append(f"1등 상금 {d.first_prize_amount:,}원")
    lines.append("상금: " + (" / ".join(prize_parts) if prize_parts else "정보 없음"))

    participation = d.participation_type or "정보 없음"
    team = d.team_config or "정보 없음"
    lines.append(f"참가 유형: {participation} / 팀 구성: {team}")
    if getattr(d, "is_career_benefit", None):
        lines.append("취업·인턴 연계: 있음")

    requirements = ", ".join(d.requirements) if d.requirements else "정보 없음"
    criteria = ", ".join(d.evaluation_criteria) if d.evaluation_criteria else "정보 없음"
    lines.append(f"지원 자격: {requirements}")
    lines.append(f"심사 기준: {criteria}")

    description = getattr(d, "description", None)
    if description:
        # description 이 dict(JSON/파이썬 repr 문자열 포함)로 오는 데이터가 있어,
        # 사람이 읽을 수 있는 항목(topic/제출 방법)만 골라 보여준다.
        raw = description if isinstance(description, str) else str(description)
        parsed = None
        for loader in (json.loads, ast.literal_eval):
            try:
                parsed = loader(raw)
                break
            except Exception:  # noqa: BLE001 - 파싱 실패 시 원문 사용
                continue
        topic, submission = None, None
        if isinstance(parsed, dict):
            topic = parsed.get("topic") or parsed.get("text") or parsed.get("content")
            submission = parsed.get("submission_method")
        else:
            topic = raw
        if topic:
            t = str(topic).strip()
            lines.append(f"소개: {t[:400]}{'…' if len(t) > 400 else ''}")
        if submission:
            s = str(submission).strip()
            lines.append(f"제출 방법: {s[:300]}{'…' if len(s) > 300 else ''}")
    if getattr(d, "url", None):
        lines.append(f"공식 링크: {d.url}")

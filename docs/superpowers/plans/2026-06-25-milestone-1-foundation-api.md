...
```bash
cd /Users/rental_mac_01/Desktop/contest-helper
git init
uv init --name contest-helper --python 3.12 --no-workspace
...
app = FastAPI(title="contest-helper")
...
```dotenv
APP_DB_URL=postgresql+psycopg://contest_helper:contest_helper@localhost:5432/contest_helper
...
services:
  db:
    ...
    environment:
      POSTGRES_USER: contest_helper
      POSTGRES_PASSWORD: contest_helper
      POSTGRES_DB: contest_helper
    ports: ["5432:5432"]
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U contest_helper"]
      interval: 5s
      retries: 5
